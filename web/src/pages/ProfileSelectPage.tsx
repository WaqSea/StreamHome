...
export function ProfileSelectPage() {
  ...
  const [ambientTheme, setAmbientTheme] = useState<ThemeId>("ember");
  ...
  return (
    <main className={`profile-gallery profile-gallery--${ambientTheme}`}>
      <EmberBackground suspendWhenHidden respectReducedMotion />
      <div className="profile-gallery__scanlines" aria-hidden="true" />
      <section className="profile-gallery__content">
        <header className="profile-gallery__header"><p>STREAMHOME / PROFILE MATRIX</p><h1>Who is watching?</h1><span>Select a server profile. Its theme shapes the complete workspace.</span></header>
        ...
        {!loading && !error && <div className="profile-gallery__grid">
          {profiles.map((profile) => {
            const profileTheme = normalizeTheme(profile.theme);
            return <article key={profile.id} className="profile-tile" onMouseEnter={() => setAmbientTheme(profileTheme)} onMouseLeave={() => setAmbientTheme("ember")} onFocus={() => setAmbientTheme(profileTheme)} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setAmbientTheme("ember"); }}>
          ...
          <button className="profile-tile profile-tile--create" onClick={() => { setShowCreate(true); setDialogError(""); }} onMouseEnter={() => setAmbientTheme("ember")}><span className="profile-create-mark">+</span><strong>Create profile</strong><small>Server profile</small></button>
        </div>}
      </section>

      {showCreate && <div className="profile-dialog" role="dialog" aria-modal="true" aria-label="Create profile"><form className="profile-dialog__panel" onSubmit={submitProfile}><p>NEW PROFILE</p><h2>Create a profile</h2><label><span>Name</span><input value={name} onChange={(event) => setName(event.target.value)} minLength={1} maxLength={40} required autoFocus /></label><ThemeChoices value={theme} onChange={setNewTheme} />{dialogError && <p className="profile-dialog__error" role="alert">{dialogError}</p>}<div className="profile-dialog__actions"><button type="submit" disabled={saving}>{saving ? "Creating..." : "Create"}</button><button type="button" onClick={() => setShowCreate(false)}>Cancel</button></div></form></div>}

      {editing && <div className="profile-dialog" role="dialog" aria-modal="true" aria-label={`Edit ${editing.name}`}><form className="profile-dialog__panel profile-dialog__panel--edit" onSubmit={submitEdit}><p>PROFILE SETTINGS</p><h2>Edit profile</h2><label><span>Name</span><input value={editName} onChange={(event) => setEditName(event.target.value)} minLength={1} maxLength={40} required autoFocus /></label><ThemeChoices value={editTheme} onChange={setEditTheme} />{dialogError && <p className="profile-dialog__error" role="alert">{dialogError}</p>}<div className="profile-dialog__actions"><button type="submit" disabled={saving || deleting}>{saving ? "Saving..." : "Save changes"}</button><button type="button" onClick={() => setEditing(null)} disabled={saving || deleting}>Cancel</button></div>{editing.id === "1" ? <p className="profile-delete-note">The administrator profile cannot be deleted.</p> : <section className="profile-delete-zone"><h3>Delete profile</h3><p>Type <strong>{editing.name}</strong> to permanently delete this profile.</p><input aria-label="Confirm profile name" value={deleteConfirmation} onChange={(event) => setDeleteConfirmation(event.target.value)} /><button type="button" disabled={deleting || saving || deleteConfirmation !== editing.name} onClick={() => void removeEditingProfile()}>{deleting ? "Deleting..." : "Delete profile"}</button></section>}</form></div>}
    </main>
  );
}
