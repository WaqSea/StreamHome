...
function EmberMediaCard({ movie, session, showReason, position, onOpen, onResume }: { movie: Movie; session?: PlaybackSession; showReason: boolean; position: number; onOpen: (movie: Movie) => void; onResume?: (movie: Movie, session: PlaybackSession) => void }) {
  ...
  return <div ref={exposureRef} className="ember-media-card-shell" data-resumable={Boolean(session && onResume) || undefined}><button type="button" className="ember-media-card" data-availability={mediaAvailability(movie)} onClick={() => onOpen(movie)}><span className="ember-media-card__tilt"><span className="ember-media-card__art"><MediaArtwork src={movie.thumbnailUrl} alt={movie.title} media={movie} className="h-full w-full object-cover" /><i aria-hidden="true" /><AvailabilityBadge movie={movie} variant="ember" /></span><span className="ember-media-card__copy"><strong>{movie.title}</strong><small>{movie.type} / {movie.releaseYear || "year n/a"}</small>{showReason && <RecommendationReason movie={movie} />}</span></span>{session && <ProgressBar className="ember-media-card__progress" progress={completionFraction(session.completionRate)} />}</button>{session && onResume && <button type="button" className="ember-media-card__resume" aria-label={`Resume ${movie.title} from ${formatDuration(session.timestamp)}`} data-tooltip={`Resume from ${formatDuration(session.timestamp)}`} onClick={() => onResume(movie, session)}><MediaActionIcon name="resume" /></button>}</div>;
}

function EmberRail({ label, title, items, sessions, showReasons = false, onOpen, onResume }: { label: string; title: string; items: Movie[]; sessions: PlaybackSession[]; showReasons?: boolean; onOpen: (movie: Movie) => void; onResume?: (movie: Movie, session: PlaybackSession) => void }) {
  const { rail, scroll, direction, proximity, proximityHandlers, canScrollPrevious, canScrollNext } = useAnimatedRail();
  if (!items.length) return null;
  return <motion.section variants={CONTENT_REVEAL} className="ember-rail" data-rail-direction={direction}><header><div><p>{label}</p><h2>{title}</h2></div><span>{String(items.length).padStart(2, "0")} catalog records</span></header><div className="ember-rail__frame" data-edge-proximity={proximity} {...proximityHandlers}><button className="ember-rail__blade ember-rail__blade--previous" disabled={!canScrollPrevious} onClick={() => scroll(-1)} aria-label={`Scroll ${title} backward`}>‹</button><div ref={rail} className="ember-rail__track">{items.map((movie, position) => <EmberMediaCard key={movie.id} movie={movie} session={sessions.find((session) => session.movieId === movie.id)} showReason={showReasons} position={position} onOpen={onOpen} onResume={onResume} />)}</div><button className="ember-rail__blade ember-rail__blade--next" disabled={!canScrollNext} onClick={() => scroll(1)} aria-label={`Scroll ${title} forward`}>›</button></div></motion.section>;
}
...
function EmberCatalogView({ query, controller, onOpen, onPlay, onResume, onCategory }: { query: AppQueryState; controller: ThemeApplicationProps["controller"]; onOpen: (movie: Movie) => void; onPlay: (movie: Movie) => void; onResume: (movie: Movie, session: PlaybackSession) => void; onCategory: (category: string) => void }) {
  ...
  return <div className={`${view === "home" ? "ember-home" : "ember-discovery"} ember-category-discovery`} data-category-mode={model.mode}>
    ...
    <AnimatedState stateKey={`${model.mode}:${model.activeCategory}`}>
      {model.gridItems.length > 0 ? <EmberGrid title={model.mode === "all" ? "All Releases" : model.activeLabel} label={model.mode === "all" ? "COMPLETE SERVER CATALOG" : "CATEGORY CATALOG"} items={model.gridItems} total={model.total} sessions={controller.sessions} showReasons={model.mode !== "all"} onOpen={onOpen} /> : hasResults ? <motion.div variants={CONTENT_STAGGER} initial="hidden" animate="shown" className={collectionsClass}>{model.sections.map((collection) => <EmberRail key={collection.id} label={collection.label} title={collection.title} items={collection.items} sessions={controller.sessions} showReasons={collection.showReasons} onOpen={onOpen} onResume={collection.id === "continue-watching" ? onResume : undefined} />)}</motion.div> : <div className="ember-category-discovery__empty"><EmberStatePanel code="NO CATEGORY SIGNAL" title={`No ${model.activeLabel} titles`} body={`No titles match the ${model.activeLabel} category.`} /></div>}
    </AnimatedState>
...
export function EmberDashboard({ query, controller, presentation }: ThemeApplicationProps) {
  ...
  const resumeWatch = (movie: Movie, session: PlaybackSession) => appNavigate("watch", { media: movie.type === "series" && session.episodeId ? session.episodeId : movie.id });
