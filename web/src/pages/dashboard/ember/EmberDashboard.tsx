...
function EmberMediaCard({ movie, session, showReason, onOpen }: { movie: Movie; session?: PlaybackSession; showReason: boolean; onOpen: (movie: Movie) => void }) {
  return <button className="ember-media-card" data-availability={mediaAvailability(movie)} onClick={() => onOpen(movie)}><span className="ember-media-card__tilt"><span className="ember-media-card__art"><MediaArtwork src={movie.thumbnailUrl} alt={movie.title} media={movie} className="h-full w-full object-cover" /><i aria-hidden="true" /><AvailabilityBadge movie={movie} variant="ember" /></span><span className="ember-media-card__copy"><strong>{movie.title}</strong><small>{movie.type} / {movie.releaseYear || "year n/a"}</small>{showReason && <RecommendationReason movie={movie} />}</span></span>{session && <ProgressBar className="ember-media-card__progress" progress={completionFraction(session.completionRate)} />}</button>;
}
