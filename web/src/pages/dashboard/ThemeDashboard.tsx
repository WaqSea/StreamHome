...
function MediaCard({ movie, session, theme, showReason, onOpen }: { movie: Movie; session?: PlaybackSession; theme: string; showReason: boolean; onOpen: (movie: Movie) => void }) {
  return <motion.button layout="position" className="catalog-card" data-card-theme={theme} data-availability={mediaAvailability(movie)} onClick={() => onOpen(movie)}>
