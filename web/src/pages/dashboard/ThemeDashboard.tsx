...
function MediaCard({ movie, session, theme, showReason, position, onOpen, onResume }: { movie: Movie; session?: PlaybackSession; theme: string; showReason: boolean; position: number; onOpen: (movie: Movie) => void; onResume?: (movie: Movie, session: PlaybackSession) => void }) {
  ...
  return <motion.div layout="position" ref={exposureRef} className="catalog-card-shell" data-card-theme={theme} data-resumable={Boolean(session && onResume) || undefined}><button type="button" className="catalog-card" data-card-theme={theme} data-availability={mediaAvailability(movie)} onClick={() => onOpen(movie)}>
