...
function MediaCard({ movie, session, theme, showReason, position, onOpen, onResume }: { movie: Movie; session?: PlaybackSession; theme: string; showReason: boolean; position: number; onOpen: (movie: Movie) => void; onResume?: (movie: Movie, session: PlaybackSession) => void }) {
  ...
  return <motion.div ref={exposureRef} className="catalog-card-shell" data-card-theme={theme} data-resumable={Boolean(session && onResume) || undefined}><button type="button" className="catalog-card" data-card-theme={theme} data-availability={mediaAvailability(movie)} onClick={() => onOpen(movie)}>
...
function CatalogDiscoveryView({ query, controller, theme, variant, onOpen, onPlay, onResume, onCategory }: { query: AppQueryState; controller: CatalogController; theme: string; variant: string; onOpen: (movie: Movie) => void; onPlay: (movie: Movie) => void; onResume: (movie: Movie, session: PlaybackSession) => void; onCategory: (category: string) => void }) {
  ...
  return <div className={`${view === "home" ? "home-view" : "browse-discovery"} category-discovery`} data-category-mode={model.mode}>
    ...
    <AnimatedState stateKey={`${model.mode}:${model.activeCategory}`}>
      {model.gridItems.length > 0 ? <CatalogGrid title={model.mode === "all" ? "All Releases" : model.activeLabel} label={model.mode === "all" ? "COMPLETE SERVER CATALOG" : "CATEGORY CATALOG"} items={model.gridItems} total={model.total} sessions={controller.sessions} theme={theme} showReasons={model.mode !== "all"} onOpen={onOpen} /> : hasResults ? <motion.div variants={CONTENT_STAGGER} initial="hidden" animate="shown" className={collectionsClass}>{model.sections.map((collection) => <MediaCollection key={collection.id} label={collection.label} title={collection.title} items={collection.items} sessions={controller.sessions} theme={theme} showReasons={collection.showReasons} onOpen={onOpen} onResume={collection.id === "continue-watching" ? onResume : undefined} />)}</motion.div> : <div className="category-discovery__empty"><EmptyState title={emptyTitle} body={`No titles match the ${model.activeLabel} category.`} /></div>}
    </AnimatedState>
