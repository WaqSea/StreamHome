...
function MediaCard({ movie, session, theme, showReason, position, onOpen, onResume }: { movie: Movie; session?: PlaybackSession; theme: string; showReason: boolean; position: number; onOpen: (movie: Movie) => void; onResume?: (movie: Movie, session: PlaybackSession) => void }) {
  ...
  </button>{session && onResume && <button type="button" className="catalog-card__resume" onClick={() => onResume(movie, session)}>Resume</button>}{showReason && feedback && <RecommendationFeedback compact movieId={movie.id} preference={feedback.preferences[movie.id] ?? null} onChange={feedback.onChange} />}</motion.div>;
}
